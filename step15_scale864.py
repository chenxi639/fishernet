"""Frozen epoch-6 neural head: add scale 864 using verified existing caches."""
import json,time
...
def main():
 ...
 previous_run=ROOT/'runs/step14_688_20260921_172409'
 checkpoint=Path(prior['checkpoint']);digest=sha(checkpoint);assert digest==prior['checkpoint_sha256']
 run=ROOT/'runs'/f'step15_864_{datetime.now():%Y%m%d_%H%M%S}';run.mkdir()
 start=time.perf_counter();print('RUN',run,flush=True)
 cfg=dict(checkpoint=str(checkpoint),checkpoint_sha256=digest,source_cache=[str(old),str(previous_run)],scales=[480,576,688,864],
 ...
 try:
  features={}
  assert json.loads((previous_run/'config.json').read_text())['checkpoint_sha256']==digest
  for scale in (480,576,688):
   folder=old if scale in (480,576) else previous_run
   path=folder/f'val_{scale}.npy';meta=json.loads((folder/f'val_{scale}_complete.json').read_text());assert sha(path)==meta['sha256']
  ...
  assert json.loads((previous_run/'val_ids.json').read_text())==ids
  np.testing.assert_array_equal(np.load(previous_run/'val_labels.npy'),labels)
  ...
  ds={s:VOCPatchDataset(root,'val',s) for s in (480,576,688,864)}
  ...
  with torch.inference_mode():
   ...
   if device=='cuda':torch.cuda.reset_peak_memory_stats()
   x=np.lib.format.open_memmap(run/'val_864.npy',mode='w+',dtype=np.float32,shape=(len(ids),16384));t=time.perf_counter()
   for j in range(0,len(ids),2):
    images=[ds[864][i]['image'].to(device) for i in range(j,min(j+2,len(ids)))]
    ...
    if (j+2)%512==0 or j+2>=len(ids):
     progress=dict(processed=min(j+2,len(ids)),total=len(ids),phase='extract864',elapsed_seconds=time.perf_counter()-start)
     dump(run/'progress.json',progress);print('PROGRESS',progress,flush=True)
   sync();seconds=time.perf_counter()-t;x.flush();del x
  full_peak=torch.cuda.max_memory_allocated()/1024**2 if device=='cuda' else None
  dump(run/'val_864_complete.json',dict(sha256=sha(run/'val_864.npy'),shape=[len(ids),16384],seconds=seconds))
  features[864]=np.load(run/'val_864.npy',mmap_mode='r')
  for j in range(0,len(ids),128):np.testing.assert_allclose(np.linalg.norm(features[864][j:j+128],axis=1),1,atol=2e-6)
  variants={};score_cache={}
  for name,scales in [('triple',[480,576,688]),('single864',[864]),('quad',[480,576,688,864])]:
  ...
  previous=np.load(previous_run/'triple_predictions.npz');np.testing.assert_allclose(previous['scores'],score_cache['triple'],rtol=0,atol=0)
  assert sha(checkpoint)==digest
  best=max(('triple','quad'),key=lambda k:variants[k]['val_map'])
  ...
  result=dict(status='completed',variants=variants,recommended=best,benchmark=timing,full_864_extraction_seconds=seconds,full_peak_allocated_mib=full_peak,
   neural_updates=0,svm_fits=0,checkpoint_unchanged=True,triple_predictions_exact=True,feature_norms_verified=True,
